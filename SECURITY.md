...
- **Upload photos** : types MIME et extension autorisés (JPEG, PNG uniquement), 10 Mo max par fichier, 12 photos max par requête, vérification que l’annonce appartient à l’utilisateur (JWT + BDD).
- **Upload documents vendeur** : validation par `validateDocumentFile` (images + PDF, 10 Mo max).
