...
import { listingAnnoncePath, listingPublicPathSegment, normalizeAnnoncePublicIdFromSegment } from '@/lib/listingPaths';
import {
    ...
    <div style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative' }}>
      <div
        style={{
          ...
          height: '100%',
          width: `${n * 100}%`,
          transform: `translateX(calc(-${safeIndex} * 100% / ${n}))`,
          transition: PRODUIT_PHOTO_SLIDE_TRANSITION,
...
export default function AnnonceListingPageClient() {
  ...
  const listingId = useMemo(() => {
    const seg = typeof idParam === 'string' ? idParam : Array.isArray(idParam) ? idParam[0] ?? '' : '';
    ...
    return normalizeAnnoncePublicIdFromSegment(s);
  }, [idParam]);
  ...
  const returnBackLabel = fromVendeurParam
    ...
    : returnBackHref.startsWith('/favoris')
      ? 'Retour aux favoris'
      : returnBackHref === '/' || returnBackHref.startsWith('/?')
        ? "Retour à l'accueil"
        : 'Retour au catalogue';
  ...
  /** Annonce + vendeur : ne pas dépendre de `user` / auth (sinon re-fetch au login → course + redirection catalogue). */
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        if (!cancelled) console.error('Error loading listing:', error);
      } finally {
  ...
  /** Favori : séparé pour ne pas relancer tout le chargement annonce quand Firebase hydrate l’utilisateur. */
  useEffect(() => {
    ...
    let cancelled = false;
    void getFavorite(user.uid, listing.id).then((favorite) => {
      if (!cancelled) setIsFavorited(!!favorite);
    });
    return () => {
      cancelled = true;
    };
  ...
  /** Barre d’adresse : URL « habituelle » (/annonce/10K… ou /annonce/<uuid>) une fois l’annonce chargée (uuid, lien abîmé au collage, etc.). */
  useEffect(() => {
    if (!listing?.id || loading) return;
    const canonical = listingAnnoncePath(listing);
    if (pathname === canonical) return;
    const q = new URLSearchParams(searchParams.toString());
    q.delete('returnTo');
    const qs = q.toString();
    router.replace(`${canonical}${qs ? `?${qs}` : ''}`, { scroll: false });
  }, [listing, loading, pathname, router, searchParams]);
  ...
  const handleShare = async () => {
    const segment = listing ? listingPublicPathSegment(listing) : listingId.trim();
    if (!segment) return;
    const path = `/annonce/${encodeURIComponent(segment)}`;
    /**
     * Origine du lien partagé :
     * - Sur le site réel (prod ou preview), toujours `window.location.origin` pour ouvrir le même déploiement
     *   (évite « page introuvable » si NEXT_PUBLIC_APP_URL pointe vers un autre domaine / ancienne URL).
     * - En local uniquement : NEXT_PUBLIC_APP_URL si défini, pour partager un lien vers la prod.
     */
    const publicOrigin = (() => {
    ...
    const url =
      publicOrigin && path.startsWith('/') ? `${publicOrigin}${path}` : '';
    const listingTitle = listing ? getListingDisplayTitle(listing) : '';
    /**
     * Ne pas mettre l’URL dans `text` : sur macOS / Safari / certains cibles de partage,
     * `text` et `url` sont fusionnés et l’URL devient
     * `/annonce/10K…%20Accéder%20à…` → 404.
     * Le lien correct reste dans le champ `url` du Web Share API.
     */
    const shareTextSansUrl = listing
      ? `Accéder à l'annonce ${listingTitle} sur Section Luxe.`
      : 'Section Luxe';
    try {
      if (typeof navigator !== 'undefined' && navigator.share) {
        await navigator.share({
          title: listingTitle || 'Section Luxe',
          text: shareTextSansUrl,
          url: url || undefined,
        });
      } else {
    ...
    return (
      <div className="annonce-produit-page" style={{ paddingTop: 'var(--header-height)', minHeight: '100vh' }}>
        <div className="produit-page-container" style={{ maxWidth: 'calc(1100px + 1cm)', margin: '0 auto', padding: '30px calc(24px - 1mm) 60px calc(24px - 1mm)' }}>
          <Link href={fromVendeurParam ? '/vendeur/annonces' : returnBackHref} scroll={false} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#666', marginBottom: 24 }}>
            <ArrowLeft size={16} />
            {returnBackLabel}
          </Link>
  ...
  return (
    ...
    <div className="annonce-produit-page" style={{ paddingTop: 'var(--header-height)', minHeight: '100vh' }}>
      <div className="produit-page-container" style={{ maxWidth: 'calc(1100px + 1cm)', margin: '0 auto', padding: '30px calc(24px - 1mm) 60px calc(24px - 1mm)' }}>
        {/* Back button */}
        <Link
          href={fromVendeurParam ? '/vendeur/annonces' : returnBackHref}
          scroll={false}
            ...
            <div className="hide-mobile" style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 0 }}>
              <div>
                ...
                >
                  ...
                  <div ref={etatInfoRefDesktop} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, minWidth: 0, position: 'relative' }}>
              ...
              <div style={{ marginTop: 20, borderTop: '1px solid #e5e5e7', paddingTop: 20, display: 'flex', flexDirection: 'column', gap: 0 }}>
                ...
                {seller && (
                  <div style={{ borderTop: '1px solid #e5e5e7', paddingTop: 16, marginTop: 20 }}>
                    <h2 style={{ display: 'flex', alignItems: 'center', gap: 6, lineHeight: 1, fontFamily: 'var(--font-inter), var(--font-sans)', fontSize: 19, fontWeight: 600, color: '#0a0a0a', margin: 0, marginBottom: 14 }}>
